...
def format_restriction_detail(restriction):
    ...
    # Add message excerpt and link if available
    if restriction.get("message_text"):
        ...
        if restriction.get("chat_id") and restriction.get("message_id"):
            chat_id = str(restriction["chat_id"]).replace("-100", "")
            msg_id = restriction["message_id"]
            tg_link = f"https://t.me/c/{chat_id}/{msg_id}"
            message += f"\n🔗 <a href='{tg_link}'>Go to message</a>"
...
async def show_restricted_users(message: types.Message, state: FSMContext, page: int = 0):
    ...
    chat_name = f"<a href='https://t.me/c/{str(chat_id).replace('-100', '').replace('-', '')}'>{chat_name}</a>" if chat else chat_name
...
async def show_user_restrictions(message: types.Message, state: FSMContext, page: int = 0):
    ...
    # ✅ Show 5 restrictions per page
    per_page = 5
    total_pages = (len(restrictions) + per_page - 1) // per_page
    start_idx = page * per_page
    end_idx = start_idx + per_page
    page_restrictions = restrictions[start_idx:end_idx]

    # ✅ Format all restrictions on this page
    restriction_details = "\n\n".join(
        f"({start_idx + i + 1}) {format_restriction_detail(r)}"
        for i, r in enumerate(page_restrictions)
    )
